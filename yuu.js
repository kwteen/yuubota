...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=557781502065:+55 (77) 8150-2065\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = botPrefix
blocked = []
...
async function starts() {
	...
	await nzwa.connect({timeoutMs: 30*1000})
        ...
        lolcatjs.fromString('[DEV] Bem-vindo de volta, Criador! Espero que você esteja bem-')
	...
	nzwa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo, Sinta-se em Casa *${mdata.subject}*`
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ Processado com Sucesso âœ”ï¸',
                                levelon: '✔️ *level ativado*',
				leveloff: ' ✔️  *level desativado*',
				levelnoton: '❌­ *leveling nao esta ligado*',
		...
		if (isGroupAdmins) return reply('Não tenho permissão para remover ADMS do grupo, mais se lembre eu estou com anti link ativado! ')
    ...
    nzwa.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               nzwa.sendMessage(from, 'Se ele não responder deve está ~bêbado~ ou dormindo sabe como é baiano',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
		...
		case 'infonomor':
               nzwa.updatePresence(from, Presence.composing) 
				...
				case 'testime':
					setTimeout( () => {
					nzwa.sendMessage(from, 'O tempo acabou:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return nzwa.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return nzwa.sendMessage(from, 'Cadê o texto?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'add':
					...
					break
                                        ...
                                        case 'kick':
                                                ...
                                                mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
                                                if (mentioned.length > 1) {
                                                        teks = 'deus mais nunca volte aqui :\n'
                                                        for (let _ of mentioned) {
                                                                teks += `@${_.split('@')[0]}\n`
                                                        }
                                                        mentions(teks, mentioned, true)
                                                        ...
                                                        mentions(`adeus mais nunca volte aqui : @${mentioned[0].split('@')[0]}`, mentioned, true)
                                                        nzwa.groupRemove(from, mentioned)
                                                nzwa.sendMessage(mentioned, 'Se fudeu KKKJ ˜‚', text)
                                                }
                                                break
				case 'exe':
	              nzwa.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'close':
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Música encontrada!!!*\nTítulo : ${anu.result.title}\nLink : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE ATÉ EU ENVIAR A MÚSICA, NÃO FAÇA SPAM*`
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    if (isLevelingOn) return reply('*o recurso de level já estava ativo antes*')
                    ...
                    reply(' *para ativar o level digite* leveling 1 ou 0 \n *Exemplo: ${prefix}leveling 1*')
                }
            break
                                case 'infogempa':
				...
				if (!isRegister) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('por qual motivo esta afk ?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'kpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'toma ai seu kpopzinho...'})
                                        await limitAdd(sender)
                                        break
                                case 'fml2':
				...
				case 'ttp':
					...
					reply('Sucesso limpei todos os chats :)')
					break
				case 'simi':
					if (args.length < 1) return reply('Textnya mana um?')
					teks = body.slice(5)
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('meciona o alvo que deseja clonar')
					...
					break
                                ...
                                case 'event':
                                        ...
                                        break
                                case 'register':
                                        if (isRegister) return  reply(`ã€Œ JÁ REGISTRADO ã€ \n\nvocê se registrou antes, esqueceu ?...`)
					...
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('o anti-link do grupo está ativo')
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('anti-link de grupo foi ativado com sucesso neste grupo ✔️')
						nzwa.sendMessage(from,`Atenção a todos os membros ativos, o antilink foi ativado, quando você enviar um link de grupo será expulso pelo bot`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('Mode anti link group sudah disable')
						var ini = anti.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('anti-link desativado ✔️')
					} else {
